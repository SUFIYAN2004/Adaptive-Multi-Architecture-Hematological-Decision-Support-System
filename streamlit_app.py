...
- Multi-Condition Risk Analysis Engine & Clinical Reporting
"""
...
PIPELINE_BLOCKS = [
    "Input", "Quality", "Preprocess", "Detection", "Segmentation",
    "Classification", "Augmentation", "Counting", "Morphology",
    "Risk Prediction", "Explainability", "Report",
]
...
st.markdown("""
    ...
    .footer-bar b { color: #9ca3af; }
    </style>
""", unsafe_allow_html=True)


def render_pipeline_strip(active_idx: int = 0, done_upto: int = -1):
    html = '<div class="pipeline-strip">'
    for i, name in enumerate(PIPELINE_BLOCKS):
        cls = "pipeline-step"
        if i <= done_upto:
            cls += " done"
        elif i == active_idx:
...
def _load_weights_prefer_fp16(base_name: str):
    """Checks for a compressed fp16 checkpoint first (base_name_fp16.pt), falls back
    to full precision (base_name.pt). Returns (state_dict, path_used) or (None, None)."""
    fp16_path = MODEL_DIR / f"{base_name}_fp16.pt"
    fp32_path = MODEL_DIR / f"{base_name}.pt"
    path = fp16_path if fp16_path.exists() else (fp32_path if fp32_path.exists() else None)
    if path is None:
        return None, None
    state_dict = torch.load(path, map_location=DEVICE)
    # cast any fp16 tensors back to fp32 for inference — most CPUs don't accelerate fp16 ops
    state_dict = {k: (v.float() if torch.is_tensor(v) and v.dtype == torch.float16 else v)
...
def load_segmentation_model():
    state_dict, path = _load_weights_prefer_fp16("unet_best")
    ...
    try:
        ...
        st.error(f"Error loading U-Net segmenter ({path.name if path else '?'}): {e}")
        return None


@st.cache_resource(show_spinner=False)
def load_cnn_model():
    state_dict, path = _load_weights_prefer_fp16("cnn_best")
    if state_dict is None:
        return None
    try:
        ...
        st.error(f"Error loading EfficientNetB0 ({path.name if path else '?'}): {e}")
        return None


@st.cache_resource(show_spinner=False)
def load_vit_model():
    state_dict, path = _load_weights_prefer_fp16("vit_best")
    if state_dict is None:
        return None
    try:
        import timm
        ...
        st.error(f"Error loading Vision Transformer ({path.name if path else '?'}): {e}")
        return None
...
def synthesize_clinical_interpretation(risk_pct: dict, abnormalities: list) -> str:
    ...
    condition_map = {
        ...
        'leukemia_suspicion': 'Significant blast cell count / atypical morphology requiring immediate hematology correlation',
...
st.markdown("""
    <div class="hero-container">
        ...
        <div class="hero-subtitle">Adaptive Multi-Architecture Hematological Decision Support & Microscopic Cytometry System — MSc Biotechnology, VIT</div>
    </div>
""", unsafe_allow_html=True)

render_pipeline_strip(active_idx=0, done_upto=-1)
...
with st.sidebar:
    ...
    n_active = sum(bool(m) for m in [det_model, seg_model, cnn_model, vit_model, risk_model])
    st.progress(n_active / 5, text=f"{n_active}/5 engines online")
...
if uploaded_file is not None:
    ...
    render_pipeline_strip(active_idx=1, done_upto=0)

    st.markdown("### 1. Automated Microscopic Quality Audit")
    q_cols = st.columns(5)
    for i, (check_name, (ok, detail)) in enumerate(quality_details.items()):
        badge = '<span class="badge-pass">PASS</span>' if ok else '<span class="badge-warn">WARN</span>'
        with q_cols[i]:
            st.markdown(f"""
    ...
    if analyze_clicked:
        if not all([det_model, seg_model, cnn_model, risk_model]):
            st.error("Missing model weights! Ensure trained `.pt` files reside inside `./models/` directory — check the sidebar engine status above.")
            st.stop()
        ...
        render_pipeline_strip(active_idx=9, done_upto=8)

        # KPI summary strip — quick scan before diving into tabs
        k1, k2, k3, k4 = st.columns(4)
        k1.markdown(f'<div class="glass-card"><div class="metric-value">{rbc_count + wbc_count + platelet_count}</div><div class="metric-label">Total Cells Detected</div></div>', unsafe_allow_html=True)
        k2.markdown(f'<div class="glass-card"><div class="metric-value" style="color:#3b82f6;">{total_wbc_classified}</div><div class="metric-label">WBCs Classified</div></div>', unsafe_allow_html=True)
        agree_pct = (model_agreement['agree'] / total_wbc_classified * 100) if total_wbc_classified else 0
        k3.markdown(f'<div class="glass-card"><div class="metric-value" style="color:#10b981;">{agree_pct:.0f}%</div><div class="metric-label">CNN/ViT Agreement</div></div>', unsafe_allow_html=True)
        k4.markdown(f'<div class="glass-card"><div class="metric-value" style="color:#fda4af;">{mean_nc_ratio:.2f}</div><div class="metric-label">Mean N/C Ratio</div></div>', unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        with tab_classification:
            ...
            st.markdown("Simultaneous inference across EfficientNetB0 (CNN) and Vision Transformer (ViT) architectures to verify cross-model consensus.")
            ...
            if total_wbc_classified > 0:
                st.markdown("<br>", unsafe_allow_html=True)
                st.progress(agree_pct / 100, text=f"Inter-Architecture Agreement Index: {agree_pct:.1f}%")
            if vit_model is None:
                st.info("ViT checkpoint not loaded — running CNN-only mode. Agreement metrics unavailable.")
        ...
        with tab_explainability:
            ...
            if show_gradcam and first_wbc_crop is not None:
                ...
                if cam_img is not None:
                    ...
                    st.info("Grad-CAM generation failed — check that the `grad-cam` package is installed.")
            else:
        ...
        with tab_risk:
            ...
            st.markdown("<br>", unsafe_allow_html=True)
            abnormalities_preview = flag_abnormalities(risk_features)
            ab1, ab2 = st.columns(2)
            with ab1:
                st.markdown("#### Detected Abnormalities")
                if abnormalities_preview:
                    for a in abnormalities_preview:
                        st.markdown(f'<div class="glass-card" style="text-align:left; margin-bottom:6px;">⚠️ {a}</div>', unsafe_allow_html=True)
                else:
                    st.markdown('<div class="glass-card" style="text-align:left; color:#10b981;">✅ No abnormalities flagged against reference ranges.</div>', unsafe_allow_html=True)
            with ab2:
                st.markdown("#### Clinical Interpretation")
                interp_preview = synthesize_clinical_interpretation(risk_pct, abnormalities_preview)
                st.markdown(f'<div class="glass-card" style="text-align:left;">{interp_preview}</div>', unsafe_allow_html=True)

        with tab_report:
            abnormalities = flag_abnormalities(risk_features)
            interpretation = synthesize_clinical_interpretation(risk_pct, abnormalities)

            st.markdown("### 📄 Diagnostic Report Generator")
...
Average N/C Ratio          : {mean_nc_ratio:.3f}
...
================================================================================
DISCLAIMER: This report is generated by an AI-assisted decision support
pipeline for research/demonstration purposes (MSc Biotechnology project). It
is NOT a substitute for professional medical diagnosis. All findings require
clinical correlation and confirmation by a licensed pathologist/hematologist.
================================================================================
"""
...
st.markdown("""
    <div class="footer-bar">
        <b>HemaVision AI</b> — Adaptive Multi-Architecture Hematological Decision Support System<br>
        Research / proof-of-concept demonstration · MSc Biotechnology, VIT · Not for clinical use without professional validation
    </div>
""", unsafe_allow_html=True)
